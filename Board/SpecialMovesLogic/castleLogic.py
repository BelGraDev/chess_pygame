...
class CastleLogic:
    ...
    def want_to_castle(self, step: Step) -> bool:
        prev_piece = self.board_status[step.start_cell]
    ...
    def castle(self, step: Step) -> None:
        king = self.board_status[step.start_cell]
        self.board_status[step.end_cell] = king
        del self.board_status[step.start_cell]
        ...
        if self._castle_rook(Step(rook1_cell_name, next_rook_cell_name)): return

        rook2_cell_name = map_index_to_cell(row, col - 2)
        next_rook_cell_name = map_index_to_cell(row, col + 1)
        self._castle_rook(Step(rook2_cell_name, next_rook_cell_name))

    def _castle_rook(self, step: Step) -> bool:
        rook = self.board_status.get(step.start_cell)
        if rook is not None and not rook.has_moved:
            self.board_status[step.end_cell] = rook
            del self.board_status[step.start_cell]
            return True
        return False